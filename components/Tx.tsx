...
import { writeMethod, waitAccepted, busyMessage, hasContract } from "@/lib/briefbounty";
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastCtx.Provider value={{ push, update }}>
      {children}
      <div className="pointer-events-none fixed bottom-20 right-4 z-[70] flex w-[min(92vw,360px)] flex-col gap-2">
        {toasts.map((t) => (
          <div key={t.id} className="sheet pointer-events-auto flex items-start gap-3 p-3 text-sm shadow-pop animate-tearUp">
            <FontAwesomeIcon
